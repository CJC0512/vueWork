...
<!--  
    props
    부모에서 자식 컴포넌트로 데이터를 전달하는 경우 우리는 다양한 <slot>을 이요할 수 있었다.
    하지만 slot의 경우 컨텐츠(또는 구조)를 전다랗기 위해 사용하는 것으로 단일 데이터를 전달하는 경우 적합하지 않으며
    부모에게 전달받은 데이터를 조작하는데도 어려움이 있다.
    이러한 경우 우리는 데이터를 전달하기 위한 용도로 props를 사용할 수 있다.
    (props는 자식에게 물려주면 자식 컴포넌트는 readonly(읽기만 가능)로 써야한다. vue는 자식 컴포넌트가 물려받은 값을
